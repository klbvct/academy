...
/**
 * Отримує токен з HttpOnly cookie або заголовка Authorization (fallback)
 */
export function getTokenFromRequest(request: NextRequest): string | null {
  const cookieToken = request.cookies.get('token')?.value
  if (cookieToken) return cookieToken

  return getTokenFromHeader(request.headers.get('authorization') || request.headers.get('Authorization'))
}
